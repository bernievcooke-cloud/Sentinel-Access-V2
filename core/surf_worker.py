...
def build_pdf(df: pd.DataFrame, diagnostics: dict, spot: dict, output_dir: str | Path) -> str:
    ...
    Image(generate_daily_chart(df, location_name), 18.6 * cm, 5.85 * cm),
    Spacer(1, 0.005 * cm),

    Image(generate_next_best_day_chart(df, location_name), 18.6 * cm, 5.85 * cm),
    Spacer(1, 0.005 * cm),

    Image(generate_weekly_chart(df, location_name), 18.6 * cm, 6.20 * cm),
    Spacer(1, 0.005 * cm),

    Paragraph(
        "<font size=6.8><b>Guide:</b> Good 8–10/10 | Fair 6–7/10 | Marginal 4–5/10 | Poor 0–3/10</font>",
        styles["Normal"],
    ),
]
