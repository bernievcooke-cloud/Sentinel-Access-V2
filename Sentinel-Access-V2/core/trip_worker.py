...
def calculate_fuel_cost(distance_km, fuel_consumption_per_100km=10, fuel_price_per_liter=1.80):
    ...
    """
    liters_needed = (distance_km / 100) * fuel_consumption_per_100km
...
def generate_report(location, report_type, coords, output_dir, trip_details=None):
    ...
    if trip_details:
        ...
        # Calculate fuel consumption (10L per 100km at $1.80/L AUD)
        ...
        c.setFont("Helvetica", 11)
        driving_hours = distance / 100  # Approximate 100km/hour
        recommendations = [
            f"• Estimated Driving Time: {driving_hours:.1f} hours",
            f"• Plan refueling stops approximately every 500km",
            f"• Check vehicle fuel tank capacity (ensure it can hold {liters_needed:.0f}L or plan multiple fills)",
            f"• Verify current fuel prices at destination",
            f"• Consider travel timing for fuel station availability",
        ]
